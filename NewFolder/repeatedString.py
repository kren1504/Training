...
def repetedString(string, n):
    
    aInString = string.count("a")

    if n < len(string):
        return string[:n].count("a")

    else:
        
        repetirVeces = (n // len(string))
        aAcumuladas = aInString * repetirVeces

        cantCaracteres = repetirVeces*len(string)

        dif = n - cantCaracteres

        if dif >0 :
            return aAcumuladas + string[:dif].count("a")
        else:
            return aAcumuladas
...
if __name__ == "__main__":
    print(repetedString("a",1000000000000))